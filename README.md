...
- For PDF processing, you need grobid installed. Grobid is a rather heavy collection of tools, and the easiest way to get it up and running as as follows:
    ...
    - start grobid via ```docker run -t -d --rm -p 4080:8080 lfoppiano/grobid:0.4.1-SNAPSHOT```
...
for incrementally building a library. Use ````nohup python dblp_xml-processing.py &``` to run this on the server.
